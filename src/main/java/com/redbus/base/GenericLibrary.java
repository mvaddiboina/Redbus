...
public class GenericLibrary extends Base {	
	...
	public void selectCheckboxBasedOnIndex(int chkBoxindex) throws TestFailedException, Exception {
		...
		try {
			...
			for (WebElement oListCollection : oElement) {
				if (oListCollection.isDisplayed()) {
					if (iCounter == chkBoxindex) {
						oListCollection.click();
						break;
					}
					iCounter++;
				}
	 ...
	 * @throws Exception
	 */
	public void selectValueFromDropDownByValue(By dropDownPath, String dropDownValue) throws TestFailedException, Exception {
		boolean bFlag = false;
		try {
